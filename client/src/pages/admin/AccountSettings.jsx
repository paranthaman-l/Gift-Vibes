...
const AccountSettings = () => {
    ...
    const handleProfileChange = async (e) => {
        ...
        if (selectedImage) {
            ...
            try {
                ...
                await AdminService.updateProfile(localStorage.getItem('uid'), data.secure_url).then((response) => {
                    toast.custom((t) => (
                        <div
    ...
    return (
        <div className='px-10 w-3/4 mt-5  h-[650px] overflow-y-scroll overflow-x-hidden'>
            ...
            <p className='text-4xl font-semibold tracking-wider text-textGray'>Account Settings</p>
                ...
                <form className='font-grotesk w-full max-xl:w-full'>
                    <div className="flex items-center mt-5">
                        ...
                        <label className="flex">
                            ...
                            <input onChange={handleProfileChange} type="file" className="cursor-pointer block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-lightGreen file:text-darkGreen hover:file:bg-opacity-60" />
                        </label>
                    </div>
                    <div className="text-darkGreen">
                        ...
                        <div className="flex my-6 flex-col ">
                            ...
                            <input id='displayName' value={user?.user?.name} type="text" className='outline-none border border-darkGreen p-3 py-4 text-sm border-opacity-30 focus:border-opacity-100 duration-200 rounded-sm' placeholder='Enter your Display name...' />
