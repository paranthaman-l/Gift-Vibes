...
const AddGift = () => {
  ...
  const AddGift = async (e) => {
    e.preventDefault();
    await AdminService.addGift(gift).then((response) => {
      navigate("/gifts")
    }).catch((err) => {
      console.log(err);
    })
  ...
  return (
    <div className='px-10 w-3/4 h-[650px] overflow-y-scroll'>
      ...
      <div className="flex justify-start relative items-center w-full mx-10">
        <form className='font-grotesk w-full max-xl:w-full' onSubmit={AddGift}>
          <div className="text-darkGreen">
            <div className="flex my-3 items-center  justify-between">
              <div className="flex flex-col w-1/2 mr-5 mt-5">
                ...
                <input onChange={handleChange} id='name' name='name' type="text" className='outline-none border border-darkGreen p-3 py-4 text-sm border-opacity-30 focus:border-opacity-100 duration-200 rounded-sm' placeholder='Enter Gift Name...' />
              </div>
              <div className="flex w-1/2 flex-col ml-5 mt-5">
                ...
                <input onChange={handleChange} id='description' name='description' type="text" className='outline-none border border-darkGreen p-3 py-4 text-sm border-opacity-30 focus:border-opacity-100 duration-200 rounded-sm' placeholder='Enter Gift Description...' />
              </div>
            </div>
            <div className="flex my-3 items-center  justify-between">
              <div className="flex flex-col w-1/2 mr-5 mt-5">
                <label className='my-1' htmlFor="price">Price<span className='text-red'>*</span></label>
                <input onChange={handleChange} id='price' name='price' type="number" className='outline-none border border-darkGreen p-3 py-4 text-sm border-opacity-30 focus:border-opacity-100 duration-200 rounded-sm' placeholder='Enter Price in INR...' />
              </div>
              <div className="flex w-1/2 flex-col ml-5 mt-5">
                <label className='my-1' htmlFor="discount">Discount <span className='text-red'>*</span></label>
                <input onChange={handleChange} id='discount' name='discount' type="number" className='outline-none border border-darkGreen p-3 py-4 text-sm border-opacity-30 focus:border-opacity-100 duration-200 rounded-sm' placeholder='Enter Discount...' />
              </div>
            </div>
            ...
            <div className="flex my-3 items-center justify-between">
              ...
              <div className="flex flex-col w-1/2 ml-5 ">
                ...
                <input onChange={handleChange} id='quantity' name='quantity' type="number" className='outline-none border border-darkGreen p-3 py-4 text-sm border-opacity-30 focus:border-opacity-100 duration-200 rounded-sm' placeholder='Enter Available Quantity...' />
              </div>
            </div>

            <div className="">
              <button type='submit' className='w-full flex '><Button title={"Add Gift"} style={"w-1/4 py-4"} /></button>
            </div>
          </div>
        </form>
      </div>
